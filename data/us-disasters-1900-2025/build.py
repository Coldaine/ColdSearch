...
<main>
  ...
  <div class="card scroll"><table id="t-torn"></table></div>

  <h2>Earthquake events</h2>
  <div class="card scroll"><table id="t-eq"></table></div>
...
function table(el, rows, cols){
  ...
  h+=rows.map(r=>'<tr>'+cols.map(c=>'<td class="'+(c.num?'num':'')+'">'+(r[c.key]??'')+'</td>').join('')+'</tr>').join('');
  el.innerHTML=h+'</tbody>';
}
table(document.getElementById('t-torn'), DATA.notable_tornadoes, [
  {key:'date',label:'Date'},{key:'name',label:'Event'},{key:'rating',label:'Rating'},
  {key:'states',label:'States'},{key:'deaths',label:'Deaths',num:1},{key:'injuries',label:'Injuries',num:1},
  {key:'notes',label:'Notes'}]);
table(document.getElementById('t-eq'), DATA.earthquakes_events, [
  {key:'date',label:'Date'},{key:'name',label:'Event'},{key:'state',label:'State'},
  {key:'magnitude',label:'Mag',num:1},{key:'deaths',label:'Deaths',num:1},{key:'injuries',label:'Injuries',num:1},
  {key:'notes',label:'Notes'}]);
