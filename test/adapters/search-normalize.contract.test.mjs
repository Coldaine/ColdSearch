...
const SEARCH_CONTRACTS = [
  {
    ...
    mockKey:
      "GET https://api.search.brave.com/res/v1/web/search?q=q&count=10&offset=0",
    handler: async () =>
      jsonResponse({
        web: {
  ...
  },
  {
    ...
    mockKey: "POST https://google.serper.dev/search",
    handler: async () =>
      jsonResponse({
  ...
  },
  {
    name: "exa",
    Adapter: ExaAdapter,
    mockKey: "POST https://api.exa.ai/search",
    handler: async () =>
      jsonResponse({
...
for (const contract of SEARCH_CONTRACTS) {
  test(`search adapter contract: ${contract.name}`, async () => {
    const restore = installFetchMock({
      [contract.mockKey]: contract.handler,
    });
    try {
